...
- Prisma Dev DB:
  - `npx prisma dev start babyai`: running
...
- API checks:
  - `GET /health` => `200` with `{"status":"ok","service":"babyai-api"}`
...
- App launch check:
  - In copy workspace (`C:\Users\milkrevenant\mobile_run\apps\mobile`):
    - `flutter run -d windows ...` reached:
      ...
      - `Built build\windows\x64\runner\Debug\babyai.exe`
      - `Syncing files to device Windows...`
    - Runtime process observed: `babyai.exe`
  - In repo path (`C:\Users\milkrevenant\Documents\code\babyai\apps\mobile`):
    - `flutter pub get`: PASS
    - `flutter build windows`: PASS
      - `Built build\windows\x64\runner\Release\babyai.exe`
